...
//				p_bullet->SetRect(this->rect_.x, rect_.y + height_frame_ * 0.25); // đây là chỗ cài vị trí của đạn
//
//			}
//			else {
...
//	}
//	else {
//		/*if(status_ == WALK_LEFT)*/  //LoadImg("img//jum_left.png", des);
...
void MainObject::RemoveBullet(const int& idx) // hàm này để xóa viên đạn mà đã va chạm
{
	int size = p_bullet_list_.size();
	if (size > 0 && idx < size) {
		BulletObject* p_bullet = p_bullet_list_.at(idx);
